  ...
  */
  ...
  function httpGet(theUrl)
  {
    ...
    // Controllo della compatibilità del browser con XMLHttpRequest
...
function ProcessRequest() 
{
    if ( xmlHttp.readyState == 4 && xmlHttp.status == 200 ) 
    {
        ...
        else
        {
            ...
            // Verifca se il JSON restituito contiene un messaggio di errore 
            if(json['ERROR'])
            {
            	alert("Protein Not Found: " + json['ERROR']);
            }
            else
            {
  ...
  // Funzione per scaricare le proteine, cambiare la variabile site per puntare ad altri repository
  function downloadProtein(){
    
    /* Indirizzo del repository locale di WebMOL da cui caricare le proteine */
    //var site = "http://cvdlab-bio.github.com/webmol/proteine/";
    //var url = site + document.getElementById('idProtein').value + ".json";
    
    /* Indirizzo di WebPDB da cui ricavare le proteine */
    var site = "http://localhost:3000/rest/protein/id/";  
    url = site + document.getElementById('idProtein').value;
